...
void VioManager::initialize_with_gt(Eigen::Matrix<double, 17, 1> imustate) {
  ...
  Cov.block(3, 3, 3, 3) =
      std::pow(0.017, 2) * Eigen::Matrix3d::Identity();                     // q
...
void VioManager::retriangulate_active_tracks(
  ...
  while (it1 != active_features.end()) {
    ...
    if (active_tracks_initializer->config().triangulate_1d) {
      success_tri =
          active_tracks_initializer->single_triangulation_1d(*it1, clones_cam);
    } else {
      success_tri =
          active_tracks_initializer->single_triangulation(*it1, clones_cam);
    }

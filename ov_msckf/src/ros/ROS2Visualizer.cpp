...
ROS2Visualizer::ROS2Visualizer(std::shared_ptr<rclcpp::Node> node,
  ...
  pub_loop_pose =
      node->create_publisher<nav_msgs::msg::Odometry>("loop_pose", 2);
  pub_loop_point =
      node->create_publisher<sensor_msgs::msg::PointCloud>("loop_feats", 2);
  pub_loop_extrinsic =
      node->create_publisher<nav_msgs::msg::Odometry>("loop_extrinsic", 2);
  pub_loop_intrinsics = node->create_publisher<sensor_msgs::msg::CameraInfo>(
      "loop_intrinsics", 2);
...
void ROS2Visualizer::visualize() {
  ...
  if (!_app->get_params().use_multi_threading_pubs) publish_images();
...
void ROS2Visualizer::publish_state() {
  ...
  for (size_t i = 0; i < poses_imu.size();
       i += std::floor((double)poses_imu.size() / 16384.0) + 1) {
...
void ROS2Visualizer::publish_images() {
  // Return if we have already visualized
  if (_app->get_state() == nullptr) return;
  if (last_visualization_timestamp_image == _app->get_state()->_timestamp &&
      _app->initialized())
    return;
...
void ROS2Visualizer::publish_groundtruth() {
  ...
  if (_sim != nullptr) {
    timestamp_inI = _app->get_state()->_timestamp +
  ...
  for (size_t i = 0; i < poses_gt.size();
       i += std::floor((double)poses_gt.size() / 16384.0) + 1) {
  ...
  Eigen::Vector3d errpos =
      state_ekf.block(4, 0, 3, 1) - state_gt.block(5, 0, 3, 1);
  double pos_nees =
  ...
  PRINT_INFO(REDPURPLE
             "error to gt => %.3f, %.3f (deg,m) | rmse => %.3f, %.3f (deg,m) | "
             "called %d times\n" RESET,
...
void ROS2Visualizer::publish_loopclosure_information() {
  ...
  _app->get_active_tracks(active_tracks_time1, active_tracks_posinG,
                          active_tracks_uvd);
  _app->get_active_image(active_tracks_time2, active_cam0_image);
  if (active_tracks_time1 == -1) return;
  if (_app->get_state()->_clones_IMU.find(active_tracks_time1) ==
  ...
  std_msgs::msg::Header header;
  header.stamp =
      ROSVisualizerHelper::get_time_from_seconds(active_tracks_time1);

  //======================================================
  // Check if we have subscribers for the pose odometry, camera intrinsics, or
  // extrinsics
  if (pub_loop_pose->get_subscription_count() != 0 ||
      pub_loop_extrinsic->get_subscription_count() != 0 ||
    ...
    cameraparams.k = {cparams(0), 0, cparams(2), 0, cparams(1),
                      cparams(3), 0, 0,          1};
  ...
  if (it_pub_loop_img_depth.getNumSubscribers() != 0 ||
      it_pub_loop_img_depth_color.getNumSubscribers() != 0) {
    // Create the images we will populate with the depths
    std::pair<int, int> wh_pair = {active_cam0_image.cols,
                                   active_cam0_image.rows};
    cv::Mat depthmap_viz;
    ...
    for (const auto &feattimes : active_tracks_uvd) {
      ...
      // Skip invalid points
      double dw = 3;
      if (uvd(0) < dw || uvd(0) > wh_pair.first - dw || uvd(1) < dw ||
          uvd(1) > wh_pair.second - dw) {
        continue;
      }
      ...
      depthmap.at<uint16_t>((int)uvd(1), (int)uvd(0)) =
          (uint16_t)(1000 * uvd(2));
      ...
      // Small square around the point (note the above bound check needs to take
      // into account this width)
    ...
    header.stamp = _node->now();
    header.frame_id = "cam0";
    sensor_msgs::msg::Image::SharedPtr exl_msg2 =
        cv_bridge::CvImage(header, "bgr8", depthmap_viz).toImageMsg();
